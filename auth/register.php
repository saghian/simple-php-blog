...
if (
    ...
    if ($_POST['password'] === $_POST['confirm']) {
        ...
        if(strlen($_POST['password']) > 5){
            ...
            $query = "SELECT * FROM `$dbName`.`users` WHERE `email` = ?";
            ...
            if ($user === false){
                ...
                $query = "INSERT INTO `$dbName`.`users` SET email = ?, first_name = ?, last_name = ?, password = ?,  `created_at` = NOW() ";
                $statement = $pdo->prepare($query);
                $statement->execute([$_POST['email'], $_POST['first_name'], $_POST['last_name'], $userPassword]);
